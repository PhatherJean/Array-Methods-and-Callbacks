...
function getFinals(data) {
    const finalized = data.filter( (item) => {
...
function getYears(data, getFinalsCB) {
    const years = getFinalsCB(data).map(function(getFinalsCB){
...
function getWinners(data, getFinalsCB){
    ...
    var gameWon = extraStep.map(function(item){
...
function getWinnersByYear(data, getWinnersCB, getYearsCB) {
   return getYearsCB(data).map((country, index) => {
        return `In ${getWinnersCB(data)[index]}, ${country} won the world cup!`
   })
...
function getAverageGoals(data) {
   const homeAvg = data.reduce((acc, element) => acc + element[`Home Team Goals`], 0);
   const awayAvg = data.reduce((acc, element) => acc + element[`Away Team Goals`], 0);
   return `${((homeAvg + awayAvg) / data.length).toFixed(2)}`
}
